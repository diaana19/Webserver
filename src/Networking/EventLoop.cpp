...
/*   Updated: 2026/03/23 12:40:26 by dirituay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void EventLoop::startUpSignal() {
	...
	signal(SIGPIPE, SIG_IGN); // don't crash if broken pipe
};
...
void EventLoop::startUpCreateEpollfd() {
	...
	if (epoll_fd == -1)
	{
		// throw std::runtime_error("Failed epoll_create1 ");
		perror("epol_ctl");
	}
};
...
void EventLoop::checkCgiReadyOrTimeOut() {
	...
	while (it != this->runningCgis.end()) {
		...
		if (it->second.readyToRead && !cgiProc.buffer.empty()) {
			// waitpid(cgiProc.pid, NULL, WNOHANG);
...
void EventLoop::readCgiOutPut(int fd, uint32_t eventFlags) {
	...
	if (eventFlags & EPOLLHUP) {
		...
		// std::cout << "[CGI] Child finished. Output:\n" s<< cgi_proc->buffer << std::endl;
...
void EventLoop::run()
{
	...
	while (g_shutdown == 0)
	{
		...
		for (int i = 0; i < n; i++) // maybe change MAXEVENT x n
		{
			int fd = events[i].data.fd;
			Server *matchListenServer = findServer_byFdClient(fd);

			if (matchListenServer){ //new client
				addNewClient(fd);
			}
			else if (fdIsCgiOutput(fd)) {
				// std::cout << "Is CgiOutput\n";
				readCgiOutPut(fd, events[i].events);
			}
			else if (events[i].events & EPOLLIN){ //Data of client 
				// std::cout << "Is handleRead\n";
				handleRead(fd); // read output with the fd  also parse the header here
			} 
			if (events[i].events & EPOLLOUT){ // Ready to write
				// std::cout << "Is handlewrite\n";
    			handleWrite(fd);
				// endingProcess(fd);
			}
...
void EventLoop::removeClient(int fd) {
	...
	//close(fd);
	this->activeRequests.erase(fd);
	// shutdown(fd, SHUT_WR); // a function to close a connection ?? but i already close it fine???
