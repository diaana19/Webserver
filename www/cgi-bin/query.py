...
<head>
<meta charset="UTF-8">
<title>Query String - Webserv</title>
...
.raw-box {{ background: rgba(232,244,253,0.6); border: 1px solid rgba(144,200,246,0.25); border-radius: 12px; padding: 14px 18px; font-family: monospace; font-size: 13px; color: #4a5a74; margin-bottom: 20px; word-break: break-all; }}
...
.param-val {{ font-size: 14px; color: #1e2d4a; font-weight: 500; }}
.empty {{ color: #be185d; font-size: 14px; background: #fce7f3; padding: 12px 16px; border-radius: 10px; }}
...
<div class="cgi-result">
  <span class="cgi-badge">GET · Query String</span>
  <h1>🔗 Query String Parser</h1>
  <div class="raw-box">QUERY_STRING: <b>{query if query else '(empty)'}</b></div>""")

if params:
    for k, v in params:
        print(f'<div class="param-row"><span class="param-key">{k}</span><span class="param-val">{v}</span></div>')
else:
    print('<div class="empty">⚠️ No query string received. Try: ?name=diana&amp;age=24</div>')
